...
export function DrawingPad({ drawing, onSave }: Props) {
    ...
    const [strokeColor, setStrokeColor] = useState<StrokeColor>(
        randomKey(strokeColors) as StrokeColor
    );
    ...
    const [backgroundColor, setBackgroundColor] =
        useState<BackgroundColor>(
            drawing
                ...
                : (randomKey(backgroundColors) as BackgroundColor)
        );
    ...
    const [selectedLeaf, setSelectedLeaf] = useState<Leaf>(
        drawing
            ? drawing.leaf_template
            : (randomKey(leafShapes) as Leaf)
    );
    ...
    function exportSvg() {
        ...
        const objSize = getObjectSizeKB(drawing);
    ...
    useHotkeys("shift+ctrl+z", redo, {
        preventDefault: true,
    });

    useHotkeys("shift+meta+z", redo, {
        preventDefault: true,
    });
        ...
        <>
            <div className="flex h-screen flex-col gap-4">
                ...
                <div className="flex flex-col gap-4 h-full bg-base-200 rounded-2xl">
                    ...
                    <div className="relative flex-1 overflow-hidden rounded-xl border bg-neutral">
                        <svg
                            ref={svgRef}
            ...
            <div
                ...
                className={`alert alert-error alert-vertical sm:alert-horizontal fixed bottom-4 left-1/2 transform -translate-x-1/2 transition-all duration-500 ease-in-out text-[#fff] ${
                    showSizeError
            ...
            >
                ...
                <div>
                    ...
                    <div className="text-xs">
                        Sorry, your leaf is a bit
                        complicated. Try erasing some
                        stuff and saving again
                    </div>
                </div>

                <button
                    className="btn btn-sm btn-secondary bg-white text-error border-0 rounded-3xl"
                    onClick={() =>
...
function UIIcon({
    ...
    return (
        <button
            className={`rounded-xl h-10 w-10 bg-base-200 text-base-100 ${
                destructive &&
    ...
    return (
        <div className="dropdown">
            <UIIcon
                ...
                bgColor={
                    swatches[
                        currentColor as keyof typeof swatches
                    ]
                }
            >
                {icon === "brush" ? (
                    <Palette className="m-auto invert mix-blend-difference" />
                ) : (
                    <PaintBucket className="m-auto invert mix-blend-difference" />
                )}
            </UIIcon>
            ...
            <ul
                ...
                style={{
                    height: `${
                        70 *
                        Math.ceil(
                            Object.keys(swatches).length / 3
                        )
                    }px`,
                }}
            >
                {Object.entries(swatches).map(
                    ([name, color]) => (
                        <div
                            key={color}
                        ...
                        >
                            <button
                                ...
                                className={`h-8 w-8 rounded-full transition-all duration-150 outline-black outline ${
                                    currentColor !== name &&
                                    "cursor-pointer hover:brightness-125"
                                }`}
                                style={{
